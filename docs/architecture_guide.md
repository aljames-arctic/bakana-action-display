...
*   **Responsibilities**:
    ...
    *   **`TabRef`**: A structured tab node class (`src/ui/tab-ref.js`) that encapsulates tab hierarchy nodes (`id`, `label`, `parent`). Automatically derives `.root` parent IDs and `.path` hierarchy arrays by traversing parent tree links.
